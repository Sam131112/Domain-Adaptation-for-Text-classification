...
def main():
            ...
            tokenized_data_mlm = load_data_tokenize("electronics/reviews.txt")
            ...
            final_train_loader = DataLoader(train_final, batch_size=8,\
                                            shuffle=True,drop_last=True)
            final_eval_loader = DataLoader(eval_final, batch_size=8,\
                                           shuffle=True,drop_last=True)
            
            final_test_loader = DataLoader(test_final,batch_size=8,\
                                           shuffle=True,drop_last=True)     
            
                      
            run_train(final_train_loader,final_eval_loader)                        
            return run_test(final_test_loader)   
        
        
test_best = []
for i in range(6):
    test_best.append(main())
print(np.mean(test_best),np.std(test_best))
