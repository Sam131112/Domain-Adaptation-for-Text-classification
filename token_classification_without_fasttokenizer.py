...
def tokenize_and_align_labels(examples):
    ...
    tokenized_inputs = tokenizer.batch_encode_plus(examples["tokens"],is_split_into_words=True,truncation=True,max_length=64,padding=True)
    length_batch = len(tokenized_inputs['input_ids'][0])
    labels = []
    word_ids_all = not_fast(examples["tokens"],length_batch)
    #print(word_ids_all)
...
def prepare_data():
        ...
        processed_datasets = dataset.map(tokenize_and_align_labels,batched=True,\
