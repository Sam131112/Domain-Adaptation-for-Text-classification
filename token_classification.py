...
def tokenize_and_align_labels(examples):
    ...
    tokenized_inputs = tokenizer(examples["tokens"],is_split_into_words=True,truncation=True,max_length=64,padding=True)
    labels = []
...
def prepare_data():
        ...
        label_list = dataset_trg['train'].features[f"{task}_tags"].feature.names
        ...
        processed_datasets_src.remove_columns_(['chunk_tags', 'id','tokens'])
...
for _ in range(1):
            ...
            fname = "broadcast_print.p"
            pickle.dump(output,open(f'baseline_ontonotes/tapt/{fname}',"wb"))
